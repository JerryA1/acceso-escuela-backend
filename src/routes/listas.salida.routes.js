...
const LISTA_SALIDA = Schema.default(db).lista_salida;
...
router.get('/', async (req, res) => {
    const listas = await LISTA_SALIDA.findAll({
        attributes: ['id', 'hora', 'comentario', 'salida'],
        include: [{model: Fecha}, {model: Alumno}, {model: Registrador}, {model: Tutor}]
      });    
    res.json(listas);
});

router.get('/alumno/:id', async (req, res) => {
    const findLista = await LISTA_SALIDA.findAll({ 
        attributes: ['id', 'hora', 'comentario', 'salida'],
        include: [{model: Fecha}, {model: Alumno}, {model: Registrador}, {model: Tutor}],
    ...
    if (findLista == 0) {
        ...
        // console.log(findLista instanceof LISTA_SALIDA); // true
        res.json(findLista);
    }
});
...
router.get('/alumno/fecha/:id_a/:id_f', async (req, res) => {
    const findLista = await LISTA_SALIDA.findOne({ 
        attributes: ['id', 'hora', 'comentario', 'salida'],
        include: [{model: Fecha}, {model: Alumno}, {model: Registrador}, {model: Tutor}],
...
router.post('/', async (req, res) => {
    ...
    const newAlumno = await LISTA_SALIDA.create({
        id_fecha: id_fecha,
        id_alumno: id_alumno,
